    ...
    // render view in views directory
        ...
        function getMenuItems(callback) {
			...
			if(user_id == 1){
				...
				var query = "SELECT CONCAT('{', '\"position\": ', m.position, ', ', '\"type\": \"', m.type, '\"',  ', ',  '\"name\": \"', m.name, '\"',  ', ', '\"icon\" :\"', m.icon, '\"',  ', ', '\"href\" :\"', m.href, '\"',  ', ', '\"class\" :\"', m.class, '\"',  IF(m.count = 1, CONCAT(', \"count\" : ', m.count), \"\"), '}') as var_val FROM modules m ORDER BY m.position";
